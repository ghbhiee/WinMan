...
- 预览显示期间按数字键 1–9 直接打开对应序号的窗口（主键盘与小键盘均可）；按键及其抬起事件都被拦截，不会传给前台应用。
- 预览有效区域改为「以 Dock 图标为起点朝面板方向的扇形」：鼠标在 Dock 行里斜向/向上朝面板运动算正在进入，横向平移到相邻图标即视为离开并在 0.2s 内关闭；不必再把鼠标挪很远才能取消预览。
- 新增菜单栏「检查更新…」：读取 GitHub Releases 最新版本，下载 universal zip、校验签名与 bundle ID 后原地替换并重启。
- 预览卡片：三个控制按钮移到标题行右侧并改为单色线框图标；标题前加序号；内建屏不再显示屏幕标签，只有副屏显示名称。
- 修复点击 Dock 图标后「移开再迅速回来」不出预览：抑制期内清空悬停项，回来即视为重新进入；抑制时长 1.5s → 1.0s。
- 修复斜着滑向远处的预览卡片时面板消失：途中扫过其他 Dock 图标（含沿 Dock 横滑到面板正下方的图标）不再撤销当前面板（切到其他白名单应用需在其图标上停留 0.3 秒），面板内判定不再依赖瞬时悬停项。
- 预览行宽度不再限制 760pt，窗口再多也一次排开（最多到屏幕宽度），不用横向滚动。
- 修复久放后预览「第一次点击无效」：面板不再为点击抢 key，卡片直接接收首次点击。
- 修复久放后预览「一闪就没/不出现」：窗口枚举与截图全部移到后台线程（App Nap 中的应用每次 AX 调用可拖到 0.25s，原先会堵住主线程让 event tap 丢事件），且构建期间不再因指针离开图标而取消显示。
